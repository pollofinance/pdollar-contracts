...
contract ShareWrapper {
    ...
    function stake(uint256 amount) public virtual {
        ...
        _totalSupply = _totalSupply.add(amount).sub(depositFee);
        _balances[msg.sender] = _balances[msg.sender].add(amount).sub(depositFee);
        share.safeTransferFrom(msg.sender, address(this), amount);
        share.safeTransfer(devFund, depositFee);
    }
...
contract Boardroom is ShareWrapper, ContractGuard {
    ...
    function initialize(
        ...
        require( _depositFeeBP <= 1000, "can't be more than 10%" );
        dollar = _dollar;
        share = _share;
        treasury = _treasury;
